...
def stretch_input(input_matrix,window_size = 5,padding=(0,0),stride=(1,1)):
    ...
    iter = 0
    for i in range( int((input_shape[2] + 2*padding[0] - window_size) / stride[0] + 1)): #input_shape[2]-window_size + 1 ):
        for j in range(int((input_shape[3] + 2*padding[1] - window_size) / stride[1] + 1)): #( input_shape[3]-window_size + 1 ):
